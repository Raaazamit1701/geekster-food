...
const Restorent = () => {
  ...
  const [res_data,setRes_data] = useState(restorent_data);
  useEffect(()=>{
    const filter_data = restorent_data.filter((e)=>{
        return (e.name.toLowerCase().includes(input.toLowerCase()) && e.rating>=num);
    })
    setRes_data(filter_data);
  },[input,num])
  ...
  return (
    <>
      <input
        type="text"
        className=" mx-10 my-10 border border-green-300 "
        ...
        {res_data.map((e, idx) => {
          ...
          return (
            <>
              <div
                key={idx}
                className="border border-gray-200 rounded-lg shadow-md m-5 md:w-[45%] lg:w-[25%]"
              >
                <div className="p-4">
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm md:text-base text-gray-700">
                    {e.outcode} {e.postcode}
                  </p>
                </div>
                <div className="bg-gray-800 text-white p-4 flex flex-col md:flex-row justify-between items-center md:items-stretch m-5 rounded-lg">
                  <p className="mb-2 md:mb-0">{e.type_of_food}</p>
                  <a href={e.URL} className="text-teal-500 hover:text-teal-400">
                    Link
                  </a>
                </div>
              </div>
